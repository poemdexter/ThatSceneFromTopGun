...
public class JumpPhysics2D : MonoBehaviour
{
    ...
    {
        // button press
        return Input.GetButtonDown(tag + "_Button1");
    }

    bool RemovedJumpInputCheck()
    {
        // let go of button
        return Input.GetButtonUp(tag + "_Button1");
    }
    ...
    bool RaycastCollideVertical(Vector2 direction)
    {
        ...
        RaycastHit2D hitLeftSide = Physics2D.Raycast((Vector2)transform.position + collisionRayLeftOffset, direction, this.renderer.bounds.extents.y + directionDepth, collidingLayerMask);
        RaycastHit2D hitRightSide = Physics2D.Raycast((Vector2)transform.position + collisionRayRightOffset, direction, this.renderer.bounds.extents.y + directionDepth, collidingLayerMask);
    ...
    void DebugRays()
    {
        ...
        Debug.DrawLine(dfloorRay_L.origin, dfloorRay_L.origin - new Vector2(0, this.renderer.bounds.extents.y + collisionRayDepthUp), Color.magenta);
        Debug.DrawLine(dfloorRay_R.origin, dfloorRay_R.origin - new Vector2(0, this.renderer.bounds.extents.y + collisionRayDepthUp), Color.magenta);
        Debug.DrawLine(ufloorRay_L.origin, ufloorRay_L.origin + new Vector2(0, this.renderer.bounds.extents.y + collisionRayDepthDown), Color.red);
        Debug.DrawLine(ufloorRay_R.origin, ufloorRay_R.origin + new Vector2(0, this.renderer.bounds.extents.y + collisionRayDepthDown), Color.red);
    }
}